 ...
 * @param {object} opt          可选参数：{ ocrUrl, savePath, captureDelayMs, debug }
...
function ocrOffsetClick(targetText, moveDir, moveUnits, threshold, opt) {
    ...
    var width = best.x2 - best.x1;        // 文字框宽度
    var height = best.y2 - best.y1;       // 文字框高度（这里备用）

    // 基准点：右边界中点
    var baseX = best.x2;                  // 右边界 x
    var baseY = best.y1 + height / 2;     // 垂直居中的 y

    // 偏移距离 = 宽度 * moveUnits
    var dist = width * moveUnits;
    ...
    // 这里使用 click()。如果你的无障碍服务当前可用，这里就能点击；
    // 如果此处仍抛“无障碍未运行”的错误，那说明系统层无障碍确实有问题，需要先去系统设置里关闭再开启一次无障碍服务，或重启手机。
    click(clickX, clickY);
