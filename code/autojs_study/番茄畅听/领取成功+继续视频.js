 ...
 * @returns {object|null} 找到的控件对象，包含bounds信息和控件属性
 */
                ...
                try {
                    ...
                    if (control && control.bounds) {
                        ...
                        if (bounds && bounds.left != null && bounds.top != null && bounds.right != null && bounds.bottom != null) {
                            ...
                            console.log("通过textContains找到控件：" + text);
                            return {
                                type: "control",
                                text: text,
                                desc: desc,
                    ...
                    console.log("控件访问异常，继续查找下一个：" + e.message);
                }
            }
        }
                ...
                try {
                    ...
                    if (control && control.bounds) {
                        ...
                        if (bounds && bounds.left != null && bounds.top != null && bounds.right != null && bounds.bottom != null) {
                            ...
                            var id = control.id();
                            var className = control.className;
                            console.log("通过descContains找到控件：" + desc);
                            return {
                                type: "control",
                                text: text,
                                desc: desc,
...
function ocrOffsetClick(targetText, moveDir, moveUnits, threshold, opt) {
    ...
    if (control) {
        console.log("已检测到此控件：" + control.text);
        console.log("控件属性：text='" + control.text + "', desc='" + control.desc + "', id='" + control.id + "', className='" + control.className + "'");

        var bounds = control.bounds;
        var textWidth = bounds.right - bounds.left;
        var textHeight = bounds.bottom - bounds.top;
        var charWidth = textWidth / targetText.length;
...
ocrOffsetClick("领取奖励", "left", 1, 0.8, {
