...
function findControlWithText(targetText) {
    ...
    try {
            ...
            for (var i = 0; i < textControls.length; i++) {
                try {
                    ...
                    if (control && control.bounds) {
                        ...
                        console.log("  id=", id);
                        console.log("  className=", controlClassName);
        ...
        try {
            ...
            console.log("遍历控件失败，跳过此方法：" + e.message);
        }
    } catch (e) {
...
function ocrOffsetClick(targetText, moveDir, moveUnits, threshold, opt) {
    ...
    if (control) {
        ...
        var textContent = control.text || "";
        var charCount = textContent.length || 1;
    ...
    var actualText = best.text || "";
    var actualTextLength = actualText.length || 1;
    var charWidth = (best.x2 - best.x1) / actualTextLength;
...
ocrOffsetClick("文件", "right", 0, 0.8, {
    //ocrOffsetClick("抖音极速版", "left", 1, 0.8, {
    //ocrUrl: "http://100.126.156.91:1224/api/ocr",
    // 注释掉这行，使用代码中默认的地址
    //ocrUrl: "http://192.168.2.100:1224/api/ocr",  // 本地 HP Pavilion
