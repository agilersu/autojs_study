...
function ocrOffsetClick(targetText, moveDir, moveUnits, threshold, opt) {
    ...
    try {
        ...
        console.error("OCR请求失败：网络连接错误 - " + e);
        console.error("请检查：");
        console.error("  1. OCR服务是否在 192.168.2.100:1224 运行");
        console.error("  2. 设备和OCR服务器是否在同一局域网");
        console.error("  3. 防火墙是否允许连接");
        console.error("当前OCR地址：" + ocrUrl);
        return false;
    }
...
ocrOffsetClick("通讯录", "up", 9, 0.8, {
    ocrUrl: "http://192.168.2.100:1224/api/ocr",
...
ocrOffsetClick("手机联系", "left", 1, 0.8, {
    ocrUrl: "http://192.168.2.100:1224/api/ocr",
    savePath: "/sdcard/待识别图片.jpg",
    captureDelayMs: 10500,
    debug: false,
    randomRange: 0.5
});
